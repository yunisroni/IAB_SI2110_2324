...
data.train <- data[index_train,] # 70% untuk data training
...
modelForest
```
Dari mode dapat dilihat bahwa tingkat kesalahan adalah 22.44%, dengan akurasi sebesar 77.56%
...
head(prediksiForest, n=10)
```
```{r}
head(data.test$Outcome,10)
```

```{r}
barplot(table(data.test$Outcome))
```
